...
`upset_vp` <- function(x, plot.hp = TRUE, order.part = "effect", decreasing.part = TRUE, order.var = TRUE, decreasing.var = TRUE, cutoff = -1, nVar = 30, col.width = 0.6, pch.size = 3, line.lwd = 0.5, show.effect = TRUE, effect.cex = 2.7, title.cex = 10, axis.cex = 8, height.ratio = c(2, 1), width.ratio = c(1, 3), int.var = NULL, pal = NULL) {
  ...
  p.vp <- ggplot2::ggplot(data = Var.part, aes_string(x = 'Var', y = 'Fractions', fill = 'Code')) +
    ...
    ggplot2::theme(panel.grid = element_blank(),
                   ...
                   axis.title = element_text(color = 'black', size = title.cex, face = "bold"),
                   plot.title = element_text(hjust = 0.5, size = title.cex, face = "bold"),
  ...
  if (show.effect)
    p.vp$layers[[2]] <- ggplot2::geom_text(aes_string(label = 'Fractions', vjust = ifelse(Var.part$Fractions >= 0, -0.2, 1.2)), color = 'black', size = effect.cex, hjust=-0.1,vjust=-0.05,angle=45)
  ...
  p.exp <- ggplot2::ggplot(data = Var.exp, aes_string(x = 'Var', y = 'Fractions', fill = 'Code')) +
    ...
    ggplot2::theme(panel.grid = element_blank(),
                   ...
                   axis.text.x = element_text(color = 'black', size = axis.cex, face = "bold"),
  ...
  p.panel <- ggplot2::ggplot(data = panel, aes_string(x = 'X2', y = 'X1', color = 'X5', fill = 'X4')) +
    ...
    ggplot2::theme(panel.grid = element_blank(),
                   panel.background = element_blank(),
                   axis.text = element_text(color = 'black', size = title.cex, face = "bold"),
